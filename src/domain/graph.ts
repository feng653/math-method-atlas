...
export function buildGraph(library: Library, methods: Method[], chapterId = '', allMethods = false) {
  ...
  nodes.push({ id: 'root', type: 'atlas', position: { x: -110, y: -36 },
  ...
  if (!chapterId && !allMethods) {
    ...
    chapters.forEach((chapter, index) => {
      ...
      nodes.push({ id: `chapter-${chapter.id}`, type: 'atlas',
      ...
      edges.push({ id: `branch-${chapter.id}`, source: 'root', target: `chapter-${chapter.id}`,
  ...
  chapters.forEach((chapter, index) => {
    ...
    if (!chapterId) {
      nodes.push({ id: `chapter-${chapter.id}`, type: 'atlas', position: { x: cx - 105, y: cy - 30 },
    ...
    chapterMethods.forEach((method, m) => {
      ...
      nodes.push({ id: method.id, type: 'atlas',
      ...
      edges.push({ id: `tree-${method.id}`, source: chapterId ? 'root' : `chapter-${chapter.id}`,
        ...
        target: method.id, type: 'straight', style: { stroke: color, opacity: 0.26, strokeWidth: 1.2 } });
    });
  });
