...
export function ProblemTypeDetail({ type, methods, questions, papers, onSelect, onClose }: Props) {
  ...
  return <aside className="detail-panel glass-panel" aria-label="题型详情">
    <header className="panel-header"><span className="eyebrow">PROBLEM TYPE / 题型</span>
      <button aria-label="关闭题型详情" onClick={onClose}><X size={19} /></button></header>
    <div className="detail-scroll">
      <h1>{type.title}</h1><p className="method-summary">{type.summary}</p>
      <span className="status-tag">{type.status === 'reviewed' ? '已审校题型' : '题型内容草案'}</span>
      <h2>识别题目要求</h2><ul>{type.recognition.map((text) => <li key={text}>{text}</li>)}</ul>
      <h2>解题检查顺序</h2><ol className="steps">{type.strategy.map((text) => <li key={text}>{text}</li>)}</ol>
      ...
      {type.methods.map((choice) => <div className="type-choice" key={choice.methodId}>
        ...
        <p>{choice.when}</p></div>)}
      <h2>常用二级公式 · {type.formulas.length}</h2>
