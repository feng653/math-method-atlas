...
凸割线：把凸组合不等式分别移项即得三段斜率关系，绝对值例由三角不等式成立；散度例 4+1+3=8；旋度例第三分量 1/2−(−1/2)=1；秩分解例 CB=5，A²=5A；概率界例四原子非负当且仅当 r∈[0.3,0.6]。5 张条件/步骤/公式及例题全部通过，标 reviewed。

据再读题面补 2026 第 3 题凸性定义、第 11 题散度、第 21 题秩分解关联。第 3 题排印端点问题仍在 sourceNote，不以新增方法掩盖来源错误。
