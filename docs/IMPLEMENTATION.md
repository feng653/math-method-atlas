...
2026-09-09 松紧按钮改到search-area左侧，间距8px，面板向下展开；窄屏搜索框为按钮留出左侧空间。81测试、内容/类型/行数门禁及构建通过；本次位置未做浏览器复测。


## 2026-09-09 force-lab 初学者阅读

- 203 张方法卡全部补充直觉、概念/符号解释、使用理由；例题全部结构化且至少两步。138 个例题增补具体代入公式，9 个笼统策略示例改为完整自编题；拉格朗日示例逐项展开消元、候选点、全局性与不等式复核。
- 24 章节共 49 条基础概念，章节卡提供题型入口；无题型的证明小库提供方法入口。86 个题型/触发卡复用章节概念、各方法直觉和例题；二级公式直接列表展示。
- 唯一职责入口：schema.ts 的 chapter.concepts / method.learning / question.statement；MethodLearning 展示方法导读，ChapterDetail 展示章节，MethodExample 统一分步例题，QuestionCards 统一卡内题面及一行折叠来源。
- formula-layout.ts 只拆顶层 quad/qquad，保留分组、环境和 left/right 配对；Formula 共用表格、自动换行与放大对话框。reading.css 统一阅读样式。DetailResize 在 App 维护的宽度上提供拖动与方向键操作，移动端隐藏拖动柄；松紧面板默认收起。
- CLI 继续为公式安全/渲染校验入口，增加 statement 公式和选项检查。formula-layout.test 对全库拆行后的公式渲染作回归；learning-content.test 检查全部章节、方法与题型引用的学习内容覆盖。
- 题面整理 9/408：2009-02、2010-04、2011-19、2014-03、2015-17、2018-16、2021-19、2024-17、2025-04，均为数学一。保留条件与选项、重新表述，不复制整套解析。其余仍为摘要并明确标注。题面整理与既有来源核验/方法关联核验分开，频次算法未变。
