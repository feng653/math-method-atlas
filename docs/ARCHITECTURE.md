...
画布默认占满页面，品牌/搜索/控制轻量悬浮。完整图谱和章节展开由hierarchy-layout调用elastic-layout生成力导向初始位置：所有节点间存在软化反平方排斥，连接采用随伸长增加的弹簧吸引；中心力限制范围，矩形避碰保护中文标题。固定420步预计算后消除残余重叠，结果确定；不再承诺同心层级环或主干零交叉。主干父边仍由edge-layout选择并冻结，所有语义边保留，次要连接参与较弱吸引。圆点标题26px、局部卡片标题16px；自动适配完整范围，用户可恢复100%阅读。长标题有省略与title提示，不能承诺任意屏幕同时显示所有完整标题。没有导航小地图。

useElasticGraph复用同一引擎处理拖动钉住、阻尼、暂停和减少动态；拖动放开后保留新弹簧静长，位置仅本次会话有效。后台或暂停立即停止，交互后沿用1.8秒休眠。目录点选章节、题型和方法保持挂载、展开与滚动位置，桌面详情并排；窄屏详情在目录上方，关闭详情返回原目录。禁止连接/删除/编辑内容；关系边不改变内容章节归属。prefers-reduced-motion下停止动态并取消定位动画；目录与详情仍可完成检索。
## 验证与扩展触发点
