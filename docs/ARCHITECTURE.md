...
2026-09-09 文稿节点契约：章节可用 `article` 指向概念主文；方法与题型采用严格联合 schema，允许既有结构化正文或 `article` 文稿正文两种形式。文稿是正文唯一来源，JSON 只保存稳定 ID、归属、标题摘要、考点和关系。`supersededBy` 保留旧 ID 及既有真题证据；新节点不继承旧 verified 关联。图谱默认显示新节点，但其他有效题型仍引用的旧工具继续可达。

`article-bindings.ts` 校验已发布章节每篇工具和题型都有唯一节点，标题和文稿一致，题型文稿工具链接与图谱候选边一致。概念、目录、题目页不额外伪装成方法节点。未发布章节的新稿仍进行 Markdown 链接与公式检查，完成审校和节点登记后才切换章节入口。
...
画布默认占满页面，品牌/搜索/控制轻量悬浮。完整图谱和章节展开由hierarchy-layout调用elastic-layout生成力导向初始位置：800坐标单位内所有节点间存在软化反平方排斥（不区分是否连接），空间网格跳过远处节点，连接采用随伸长增加的弹簧吸引；仅实际连线产生吸引，无独立中心力；文字不参与碰撞，固定在点下方，可能重叠。radialSeed恢复4433c88的宽大分层径向起点；连接静长为起点距离30%，限制在240—560，取消初始中心压紧。最大节点每步位移低于0.12连续30步才标记settled，4000步上限记录未收敛；不再根据文字外框移动节点。当前全部章节与全图均通过收敛测试；结果确定，但力模拟后不保证严格同心环或零交叉。主干父边仍由edge-layout选择并冻结，所有语义边保留，次要连接参与较弱吸引。所有图谱节点使用固定下置32px标题，圆点按唯一邻接点数量开方增大，上限56px；自动适配完整范围，用户可恢复100%阅读。长标题有省略与title提示，不能承诺任意屏幕同时显示所有完整标题。没有导航小地图。

useElasticGraph复用同一引擎处理拖动钉住、阻尼、暂停和减少动态；拖动放开后不重设弹簧静长；连线只产生吸引，压缩不反向推开，位置仅本次会话有效。后台或暂停立即停止，可见且启用时持续模拟，稳定后也不停；吸引/邻域排斥系数均为上一版两倍，速度保留率0.7（此前0.8），抑制惯性。目录点选章节、题型和方法保持挂载、展开与滚动位置，桌面详情并排；窄屏详情在目录上方，关闭详情返回原目录。禁止连接/删除/编辑内容；关系边不改变内容章节归属。prefers-reduced-motion下停止动态并取消定位动画；目录与详情仍可完成检索。
## 验证与扩展触发点
...
圆点模式（2026-09-08从dev/mobile-dot-graph合入main）：完整图谱以compact数据标记驱动轻量圆点标题组件，减少端口为每节点2个，使用直线边；章节概览与局部卡片保持原样。motion-scheduler在可见且启用时保持单一模拟循环，稳定后仍持续计算，加载/拖动/恢复不创建重复循环；暂停/后台立即取消。粗指针50ms、其他40ms最小模拟间隔，低于0.05世界单位的位移不更新React节点引用。休眠不代表关闭交互。dev分支只运行CI，不部署正式Pages。
